...
class DummyVideoVisionInputGenerator(DummyInputGenerator):
    """
    Generates dummy video vision inputs.
    """
    ...
    def __init__(
        self,
        task: str,
        normalized_config: NormalizedVideoVisionConfig,
        ...
        **kwargs,
        ...
        # Some vision models can take any input sizes, in this case we use the values provided as parameters.
        if normalized_config.has_attribute("num_frames"):
            self.num_frames = normalized_config.num_frames
        else:
    ...
    def generate(self, input_name: str, framework: str = "pt", int_dtype: str = "int64", float_dtype: str = "fp32"):
        if input_name == "pixel_mask":
            return self.random_int_tensor(
                shape=[self.batch_size, self.num_frames, self.height, self.width],
                max_value=1,
                framework=framework,
                dtype=int_dtype,
            )
            ...
            return self.random_float_tensor(
                shape=[self.batch_size, self.num_frames, self.num_channels, self.height, self.width],
                framework=framework,
                dtype=float_dtype,
            )
